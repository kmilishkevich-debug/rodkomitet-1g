...
// greeting_name — имя для приветствия («Виктория Петровна»), появляется
// после teacher-cabinet-setup.sql. Пока колонки нет, запрос с ней падает,
// поэтому тихо переспрашиваем по-старому — иначе учитель потеряет подпись.
export async function fetchUserRole() {
  if (!supabase) return null;
  const { data: sess } = await supabase.auth.getSession();
  const uid = sess?.session?.user?.id;
  if (!uid) return null;
  // full_name — полное имя («Наталья Коваленкова») для инициалов в кружке-аватаре,
  // появляется после committee-names-setup.sql; пока колонки нет — спрашиваем без неё
  const { data: full, error: fullErr } = await supabase
    .from("user_roles").select("role, display_name, greeting_name, full_name").eq("user_id", uid).maybeSingle();
  if (!fullErr && full) return full;
  const { data, error } = await supabase
    .from("user_roles").select("role, display_name, greeting_name").eq("user_id", uid).maybeSingle();
  if (!error && data) return data;
  const { data: old, error: oldErr } = await supabase
    .from("user_roles").select("role, display_name").eq("user_id", uid).maybeSingle();
  if (oldErr || !old) return null;
  return old;
}
...
// ===== Личная переписка учителя с семьёй =====
// Одна строка — одно сообщение, ветка собирается по family_n.
// null = таблицы family_messages ещё нет (teacher-cabinet-setup.sql не запускали),
// тогда экран молча показывает пустой список, а не ошибку.

// Вся переписка (кабинет учителя: список семей и последние сообщения)
export async function fetchFamilyMessages() {
  if (!supabase) return null;
  const { data, error } = await supabase
    ...
    .limit(500);
  if (error) {
    console.error("Supabase (переписка с семьями):", error);
    return null;
  }
  return data;
}

// Переписка одной семьи (родитель видит только свою ветку)
export async function fetchFamilyThread(familyN) {
  if (!supabase || !familyN) return null;
  const { data, error } = await supabase
    .from("family_messages")
    .select("*")
    ...
    .order("created_at", { ascending: true });
  if (error) {
    console.error("Supabase (ветка семьи):", error);
    return null;
  }
  return data;
}

// Новое сообщение: { family_n, from_teacher, author, text }
export async function sendFamilyMessage(payload) {
  if (!supabase) throw new Error("База не настроена");
  const { data, error } = await supabase
    .from("family_messages").insert(payload).select().single();
  if (error) throw error;
  return data;
}

// Отметить ветку прочитанной той или другой стороной
export async function markThreadRead(familyN, side) {
  if (!supabase || !familyN) return;
  const field = side === "teacher" ? "read_teacher" : "read_family";
  const { error } = await supabase
    .from("family_messages")
    .update({ [field]: true })
    .eq("family_n", familyN)
    .eq("from_teacher", side !== "teacher")
    .eq(field, false);
  if (error) console.error("Supabase (отметка прочтения):", error);
}

// Удалить сообщение (только вошедший по паролю)
