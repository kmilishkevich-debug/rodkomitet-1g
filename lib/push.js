...
// Важно: роль «комитет» не понижаем до «родителя» — если член комитета
// заглянул в родительский вид, его подписка должна остаться комитетской.
async function saveSubscription(sub, role) {
  if (!isLive) return { ok: false, error: "База не настроена" };
  const json = sub.toJSON();
  let newRole = role === "committee" ? "committee" : "parent";
  if (newRole === "parent") {
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (existing?.role === "committee") newRole = "committee";
  }
