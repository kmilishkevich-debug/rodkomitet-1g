...
// Рассылка: audience = "all" (все подписки), "committee" (только комитет)
// или "parents" (все, кроме классного руководителя).
...
export async function sendPushToAll({ title, body, url = "/" }, audience = "all") {
  ...
  // У родителей role часто пустая, поэтому .neq не годится — он отбросил бы и их
  if (audience === "parents") q = q.or("role.is.null,role.neq.teacher");
  ...
  let sent = 0;
  const gone = [];
  const failed = []; // ошибки доставки, которые раньше терялись молча
  await Promise.all(
    subs.map(async (s) => {
      try {
        ...
        if (e.statusCode === 404 || e.statusCode === 410) {
          ...
          failed.push({ endpoint: s.endpoint.slice(-12), status: e.statusCode || null, message: e.message });
          console.error("Пуш не доставлен:", s.endpoint.slice(-12), e.statusCode, e.message);
        }
      }
    })
  );
  if (gone.length) await sb.from("push_subscriptions").delete().in("endpoint", gone);
  return { ok: true, sent, gone: gone.length, failed: failed.length, errors: failed };
}
